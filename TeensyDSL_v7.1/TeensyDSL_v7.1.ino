 ...
 * ╠════════════════════════════════════════════════════════════════╣
 * ║  WIRING:                                                       ║
 * ║  OLED SDA/SCL  → 18/19   SD CS → built-in slot   LED → 13      ║
 * ║  BTN_UP        → pin 3 → GND                                   ║
 * ║  BTN_DOWN      → pin 4 → GND                                   ║
...
static bool loadFromSD(const char* path) {
    if (!SD.begin(BUILTIN_SDCARD)) { dbgErr("SD init failed"); return false; }
    File f = SD.open(path);
...
static void loadFileList(int osIdx) {
    ...
    if (!SD.begin(BUILTIN_SDCARD)) { dbgErr("SD init failed in file browser"); return; }
...
void setup() {
    ...
    if (!SD.begin(BUILTIN_SDCARD)) Serial.println("[SD] Warning: init failed.");
