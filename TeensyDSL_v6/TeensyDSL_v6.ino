...
enum AppState {
    ...
    ST_OS_MENU,
    ST_FILE_MENU,
    ST_VALIDATE_RESULT, // NEW: shows validator output before run
    ST_RUNNING,
    ST_CHECKPOINT,
    ST_HALTED,
    ST_STORAGE
...
static void renderStorage() {
    oled.clearDisplay();
    oledRow(0, "  STORAGE MODE     ", true);
    oledRow(2, " SD card on host"); oledRow(3, " Edit script.dsl");
    oledRow(5, " Reset to exit");    oledRow(7, " LED blinks = alive");
    oled.display();
}
...
void maybeUpdateOLED() {
    ...
    switch (appState) {
        ...
        case ST_STORAGE:         renderStorage();         break;
        default: break;
    }
}
...
static bool detectStorageMode() {
    pinMode(PIN_BTN_STORAGE, INPUT_PULLUP);
    bool held = true;
    unsigned long start = millis();
    while (millis() - start < MODE_HOLD_MS) {
        if (digitalRead(PIN_BTN_STORAGE) == HIGH) { held = false; break; }
        digitalWrite(PIN_LED, ((millis() - start) / 100) % 2);
        delay(10);
    }
    digitalWrite(PIN_LED, LOW);
    if (held) {
        for (int i=0;i<1;i++){digitalWrite(PIN_LED,HIGH);delay(800);digitalWrite(PIN_LED,LOW);}
        return true;
    }
...
static void runStorageMode() {
    appState = ST_STORAGE;
    if (!SD.begin(PIN_SD_CS)) {
        Serial.println("[STORAGE] SD init failed!"); while(true){digitalWrite(PIN_LED,HIGH);delay(100);digitalWrite(PIN_LED,LOW);delay(100);}
    }
    for (int i = 0; i < NUM_OS; i++) SD.mkdir(osFolders[i]);
    MTP.begin(); MTP.addFilesystem(SD, "TeensyDSL");
    Serial.println("[STORAGE] MTP active. Edit script.dsl, then reset Teensy.");
    if (oledReady) maybeUpdateOLED();
    while (true) {
        MTP.loop();
        static unsigned long lb = 0;
        if (millis() - lb > 1000) { lb = millis(); digitalWrite(PIN_LED, !digitalRead(PIN_LED)); }
    }
}
...
void setup() {
    ...
    if (detectStorageMode()) { runStorageMode(); return; }
