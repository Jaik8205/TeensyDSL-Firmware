...
// ── Structs declared here so Arduino's auto-generated function prototypes
//    can see them — Arduino inserts prototypes right after #include lines ──
struct ValidationReport {
    int errors;    // syntax / semantic — blocks execution
    int warnings;  // unknown commands  — allows execution with confirmation
};
...
#define OLED_ROWS           8   // 64px / 8px per text-size-1 row
...
// ─────────────────────────────────────────────────────────────────────
/*
 * Teensy 4.1 has a built-in SDIO SD slot — no SPI, no CS pin.
 * SD.begin(BUILTIN_SDCARD) is called ONCE in setup().
 * sdReady guards all subsequent SD operations.
 */
bool sdReady = false;
...
static bool safeTypeChar(char c) {
    uint16_t key = 0; bool shift = false;  // uint16_t: Teensy KEY_ macros include 0xF000 flag
...
static bool loadFromSD(const char* path) {
    if (!sdReady) { dbgErr("SD not available"); return false; }
    File f = SD.open(path);
...
static void loadFileList(int osIdx) {
    ...
    if (!sdReady) { dbgErr("SD not available"); return; }
...
void setup() {
    ...
    // Initialize SD once using Teensy 4.1 built-in SDIO slot (BUILTIN_SDCARD).
    // No SPI, no CS pin — BUILTIN_SDCARD is the correct argument for this hardware.
    sdReady = SD.begin(BUILTIN_SDCARD);
    if (!sdReady) {
        Serial.println("[SD] Warning: init failed. Insert card and reset.");
    } else {
        Serial.println("[SD] Built-in SDIO slot ready.");
